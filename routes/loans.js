...
router.get('/', (req, res, next) =>  {
  Loan.findAll({
    ...
    res.render("loans/loans", {loans: loans, category: "Loans"});
  }).catch(error => {
      res.send(500, error);
   });
});
...
router.get('/overdue', (req, res, next) =>  {
  Loan.findAll({
    ...
    res.render("loans/loans", {loans: loans, category: "Overdue Loans"});
  }).catch(error => {
      res.send(500, error);
   });
});
...
router.get('/checked_out', (req, res, next) =>  {
  Loan.findAll({
    ...
    res.render("loans/loans", {loans: loans, category: "Checked Out Loans"});
  }).catch(error => {
      res.send(500, error);
   });
});
...
router.get('/new', (req, res, next) =>  {
  ...
  Promise.all([book, patron]).then(data => {
    res.render("loans/new", {books: data[0], patrons: data[1], date: date, dateAdd: dateAdd})
  })
});
  ...
  Loan.create(req.body).then(loans => {
    res.redirect("/loans")
      ...
      if(error.name === "SequelizeValidationError") {
        ...
        const dateAdd = moment(date).add(7, 'days').format('YYYY-MM-DD');
        Promise.all([book, patron]).then(data => {
          res.render("loans/new", {books: data[0], patrons: data[1], date: date, dateAdd: dateAdd, errors: error.errors})
        })
      } else {
        throw error;
      }
